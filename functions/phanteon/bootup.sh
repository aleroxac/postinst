...
function bootup() {
	if [ -f /etc/rc.local ]; then
		...
		if $(cat /etc/rc.local | grep "rfkill block bluetooth" > /dev/null); then
			sed -i 's/^exit 0$/rfkill block bluetooth\nexit 0/g' /etc/rc.local
		fi
		...
		if $(cat /etc/rc.local | grep "echo 937"); then
    		sed -i 's|^exit 0$|'"${BRIGHTNESS}"'|g' /etc/rc.local
		fi
	else
	...
	if $(cat /etc/default/grub | grep 'GRUB_TIMEOUT="10"' > /dev/null); then
		sed -i 's|GRUB_TIMEOUT="10"|GRUB_TIMEOUT="'"${GRUBTIMEOUT}"'"|g' /etc/default/grub
		GRUB1=1
	else
		GRUB1=0
	fi
	...
	if $(cat /etc/default/grub | grep $GRUBDEFAULT) || $(cat /etc/default/grub | grep '="quiet"') > /dev/null; then
		sed -i 's|'"${GRUBDEFAULT}"'|GRUB_CMDLINE_LINUX_DEFAULT="'"$GRUBOPT"'"|g' /etc/default/grub
		GRUB2=1
	else
		GRUB2=0
	fi
